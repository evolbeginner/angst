...
if __FILE__ == $0; then
  ...
  infiles.each do |infile|
    ...
    next unless taxa.include?(b) if not taxa.empty?
  ...
  ####################################################
  multis.each_pair do |gain_or_loss, v1|
    next if gain_or_loss != :gained
    v1.each_pair do |b, v2|
      v2.each_pair do |type, v3|
        next if type != :gene
        v3.each_pair do |fam, count|
          unless consis[gain_or_loss][type][fam].include?(b)
            consis[gain_or_loss][type][fam][b] = 0
          end
          consis[gain_or_loss][type][fam][b] += gained[:gene][fam]
        end
      end
    end
  end
  ...
  consis.each_pair do |gain_or_loss, v1|
    v1.each_pair do |type, v2|
      v2.each_pair do |fam, v3|
        if rastInfo.include?(fam)
          all << rastInfo[fam].categories
          if v3.size >= 3
            selected << rastInfo[fam].categories
          end
        end
      end
    end
  ...
  freq = Hash.new
  freq[:selected] = selected.inject(Hash.new(0)) { |h,v| h[v] += 1; h }
  freq[:all] = all.inject(Hash.new(0)) { |h,v| h[v] += 1; h }


  ####################################################
  results = Parallel.map(freq[:selected], in_processes: $cpu) do |annot, f|
    num_str = [f, freq[:all][annot], freq[:selected].values.sum, freq[:all].values.sum].join(',')
    pvalue_info[annot] = `#{$FISHER} --num #{num_str} --type fisher 2>/dev/null`.chomp
    pvalue_info[annot] = pvalue_info[annot]=~/\d/ ? pvalue_info[annot].to_f : 1
    pvalue_info
  end
  ...
  fdr_info.each_pair do |annot, fdr|
    puts [annot, fdr].join("\t")
  end
